extends SceneTree
## Validates title main scene, shop/orders loop, and Mia order level config.
...
func _run() -> void:
	...
	ok = _check_order(gs.catalog.get_order(&"order_mia_001"), "Mia", &"strawberry", 20, 20, 150, 25, 5) and ok
	ok = _check_order(gs.catalog.get_order(&"order_jordan_002"), "Jordan", &"cupcake", 22, 22, 200, 35, 7) and ok
	ok = _check_order(gs.catalog.get_order(&"order_taylor_003"), "Taylor", &"chocolate", 25, 20, 275, 45, 10) and ok

	var noah = gs.catalog.get_order(&"order_noah_004")
	if noah == null or noah.all_collection_targets().size() != 2:
		push_error("Noah multi-objective missing")
		ok = false
	else:
		print("[OK] Noah has two collection objectives")

	if gs.get_order_status("order_morgan_005") != SaveData.OrderStatus.LOCKED:
		push_error("Morgan should be locked")
		ok = false
	else:
		print("[OK] Morgan locked until Candied Grapes")

	var level = gs.begin_order_level("order_mia_001")
	if level == null or level.move_limit != 20:
		...
		print("[OK] Mia configures 20 moves / 20 strawberries")

	var coins_before: int = gs.data.coins
	var stars_before: int = gs.data.stars
	gs.on_level_won("order_mia_001", 900, 10, 20)
	if gs.get_order_status("order_mia_001") != SaveData.OrderStatus.READY_TO_COMPLETE:
		push_error("not ready to complete")
		ok = false
	var rewards: Dictionary = gs.complete_order("order_mia_001")
	if rewards.is_empty() or gs.data.coins <= coins_before or gs.data.stars <= stars_before:
		push_error("complete rewards failed")
		ok = false
	elif not gs.complete_order("order_mia_001").is_empty():
		push_error("duplicate complete granted rewards")
		ok = false
	else:
		print("[OK] win → complete once")

	gs.begin_order_level("order_jordan_002")
	var c2: int = gs.data.coins
	gs.on_level_lost("order_jordan_002")
	if gs.get_order_status("order_jordan_002") != SaveData.OrderStatus.FAILED or gs.data.coins != c2:
	...
	router.pending_order_id = "order_mia_001"
	...
	if board == null or controller == null:
		...
		if cfg == null or int(cfg.move_limit) != 20:
			...
			print("[OK] main.tscn loads Mia puzzle config")
		gp.queue_free()
